...
function mapPartnershipType(value: string): string | null {
  ...
  if (v === "gifted" || v === "gifted - no ask" || v.includes("gifted")) return "gifted_no_ask";
  if (v === "gifted - soft ask") return "gifted_soft_ask";
  if (v === "gifted - deliverable ask") return "gifted_deliverable_ask";
  if (v === "gifted - recurring") return "gifted_recurring";
  return null;
}
...
async function main() {
  const campaignMap: Record<string, string> = {
    ...
    "FW25": "b0397715-54da-48b3-976f-09d6bdf13ad4", // maps to October
    "NOV25": "e7c4e86d-fe55-470a-91f5-27c802cbb2d5", // maps to November
  };
  ...
  console.log("Processing NOV25 CSV...");
  ...
  console.log(`Total unique influencers to update: ${allUpdates.size}\n`);
  ...
  for (const [handle, update] of allUpdates) {
    ...
    if (influencer.partnership_type !== update.partnershipType) {
      ...
      if (!error) {
        console.log(`Updated ${influencer.name}: ${influencer.partnership_type} -> ${update.partnershipType}`);
        influencerUpdates++;
      }
    }

...
// Get or create December 2025 campaign
async function getOrCreateDec25Campaign(): Promise<string> {
  // Check for existing December 2025 campaign
  const { data: existing } = await supabase
    .from("campaigns")
    .select("id")
    .or("name.eq.December 2025,name.eq.December 2025 Gifting,name.ilike.%dec%25%")
    .single();

  if (existing) {
    console.log("Using existing December 2025 campaign");
    return existing.id;
  }
  ...
  console.log("Created new campaign: December 2025 Gifting");
  return newCampaign.id;
}

async function main() {
  const csvPath = "/Users/shanepetersen/Downloads/Campaigns 25'-DEC25.csv";
  ...
  // Get or create December 2025 campaign
  const dec25CampaignId = await getOrCreateDec25Campaign();
  console.log(`Campaign ID: ${dec25CampaignId}\n`);
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    // Skip if partnership type is "Instructor"
    if (partnershipTypeRaw.toLowerCase() === "instructor") {
      console.log(`[${i + 1}/${rows.length}] ${name} - Skipped (Instructor)`);
      stats.skippedInstructor++;
      continue;
    }
    ...
    if (existing) {
      ...
      const { data: newInfluencer, error } = await supabase
        ...
        .insert({
          ...
          notes: notes ? `${notes}\nImported from DEC25 CSV` : "Imported from DEC25 CSV",
        })
        .select("id")
        .single();
    ...
    // Add to December 2025 campaign
    const { data: existingLink } = await supabase
      .from("campaign_influencers")
      .select("id")
      .eq("campaign_id", dec25CampaignId)
      .eq("influencer_id", influencerId)
      .single();

    if (!existingLink) {
      const { error: linkError } = await supabase.from("campaign_influencers").insert({
        campaign_id: dec25CampaignId,
      ...
      console.log(`  Already in December 2025 campaign`);
    }
  }
  ...
  console.log(`Skipped (Instructor):    ${stats.skippedInstructor}`);
  ...
  if (postUrlsForReview.length > 0) {
    console.log("\n=== POST URLs NEEDING MANUAL HANDLE LOOKUP ===");
    postUrlsForReview.forEach((p) => {
      console.log(`${p.name}: ${p.url}`);
    });

    // Save to file
    const outputPath = "/Users/shanepetersen/Downloads/dec25-post-urls.txt";

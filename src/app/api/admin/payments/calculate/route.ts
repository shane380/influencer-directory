    ...
    if (invite.has_ad_spend) {
      ...
      if (isLocked(existing)) {
        ...
        allPayments.push({
          ...
          amount_owed: amount,
          amount_paid: existing?.amount_paid ?? null,
  ...
  for (const la of (legacyAffiliates || [])) {
    ...
    if (isLocked(existingLegacy)) {
      ...
      allPayments.push({
        ...
        payment_type: "legacy_affiliate_commission",
        amount_owed: aff.amount,
        amount_paid: existingLegacy?.amount_paid ?? null,
        paid_at: existingLegacy?.paid_at ?? null,
        status: existingLegacy?.status || "pending",
        payment_method: la.payment_method || null,
        payment_detail: la.payment_detail || null,
        notes: aff.notes,
        deal_id: null,

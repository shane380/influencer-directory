...
async function loadProfiles(
  db: any, influencerIds: string[],
): Promise<Map<string, { name: string | null; handle: string | null; photo: string | null }>> {
  if (influencerIds.length === 0) return new Map();
  const { data } = await db.from("influencers").select("id, name, instagram_handle, profile_photo_url").in("id", influencerIds);
  return new Map(((data as any[]) || []).map((i) => [String(i.id), { name: i.name ?? null, handle: i.instagram_handle ?? null, photo: i.profile_photo_url ?? null }]));
}

// influencer_id -> creator_id via the invite chain (creators.invite_id ->
// creator_invites.influencer_id). Best-effort; unlinked influencers map to null.
async function resolveCreatorIds(db: any, influencerIds: string[]): Promise<Map<string, string>> {
  const out = new Map<string, string>();
  if (influencerIds.length === 0) return out;
  const { data: invites } = await db.from("creator_invites").select("id, influencer_id").in("influencer_id", influencerIds);
  const inviteToInfluencer = new Map<string, string>();
  const inviteIds = ((invites as any[]) || []).map((i) => { inviteToInfluencer.set(String(i.id), String(i.influencer_id)); return String(i.id); });

  ...
  const { data } = await supabase
    ...
    .select(
      `id, campaign_id, influencer_id, gift_products_override, gift_max_selects_override,
       ...
       campaigns!inner(name, gift_enabled, gift_hero_image, gift_blurb, gift_products, gift_max_selects),
...
export function effectiveMaxSelects(assignment: any, campaign: any): number {
  return assignment.gift_max_selects_override ?? campaign.gift_max_selects ?? 3;
}
